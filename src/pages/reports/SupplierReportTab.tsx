...
export function SupplierReportTab() {
  ...
  async function handleSearch(name: string) {
    ...
    setLoading(true);
    try {
      const res = await apiFetch(`${API}/api/Reports/supplier-report?supplierName=${encodeURIComponent(name)}`, { headers: authHeaders() });
    ...
    <div className="p-8 space-y-6">
      <div className="flex gap-3 items-end max-w-md">
        <div className="flex-1 space-y-1">
          <label className="text-xs text-muted-foreground">{t("lbl_supplierName")}</label>
          <SupplierAutocomplete value={query} onChange={v => setQuery(v)} />
        </div>
      ...
      {!loading && data && (
        <>
          <div className="flex items-center gap-3 flex-wrap">
            <p className="text-lg font-semibold text-foreground">{data.suppliername as string}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <ReportCard label={t("rc_supplyOrders")}   value={data.totalsupplycount as number} />
            <ReportCard label={t("rc_supplyAmount")}   value={`$${(data.totalsupplyamount as number).toLocaleString()}`}    color="text-primary" />
            <ReportCard label={t("rc_totalPaid")}      value={`$${(data.totalpaid as number).toLocaleString()}`}            color="text-emerald-600 dark:text-emerald-400" />
            ...
            <ReportCard label={t("rc_netDebt")}        value={`$${(data.totalnetdebt as number).toLocaleString()}`}         color="text-destructive" />
          </div>
