  ...
  async function handleSearch(name: string) {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/Reports/supplier-report?supplierName=${encodeURIComponent(name)}`,
        { headers: authHeaders() },
      );
      ...
      {!loading && data && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <ReportCard
              label={t("rc_supplyOrders")}
              value={data.totalsupplycount as number}
            />
            <ReportCard
              label={t("rc_supplyAmount")}
              value={`$${(data.totalsupplyamount as number).toLocaleString()}`}
              color="text-primary"
            />
            <ReportCard
