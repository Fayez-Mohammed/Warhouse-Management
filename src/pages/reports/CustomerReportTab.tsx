  ...
  async function handleSearch(name: string) {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/Reports/customer-report?customerName=${encodeURIComponent(name)}`,
        { headers: authHeaders() },
      );
      ...
      <div className="flex gap-3 items-end max-w-md">
        <div className="flex-1 space-y-1">
          ...
          <PersonAutocomplete
            ...
            placeholder={t("placeholder_searchCustomer")}
          />
        </div>
        <button
        ...
        <>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              ...
              dir="ltr"
            >
              #{data.usernumber as number}
            </span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <ReportCard
              label={t("rc_orders")}
              value={data.totalorderscount as number}
            />
            <ReportCard
              label={t("rc_totalSales")}
              value={`$${(data.totalsalesamount as number).toLocaleString()}`}
              color="text-emerald-600 dark:text-emerald-400"
            />
            <ReportCard
              label={t("rc_totalPaid")}
              value={`$${(data.totalpaid as number).toLocaleString()}`}
              color="text-primary"
            />
            <ReportCard
              label={t("rc_totalDebt")}
              value={`$${(data.totaldebt as number).toLocaleString()}`}
              color="text-destructive"
            />
            <ReportCard
              label={t("rc_returns")}
              value={data.totalreturnscount as number}
            />
            <ReportCard
              label={t("rc_returnsAmt")}
              value={`$${(data.totalreturnsamount as number).toLocaleString()}`}
              color="text-amber-500"
            />
            <ReportCard
              label={t("rc_unpaidReturns")}
              value={`$${(data.totalremainingunpaidreturns as number).toLocaleString()}`}
              color="text-amber-500"
            />
            <ReportCard
              label={t("rc_netDebt")}
              value={`$${(data.totalnetdebt as number).toLocaleString()}`}
              color="text-destructive"
              sub={`${t("report_lastOrder")}: ${formatDate(data.lastorderdate as string)}`}
            />
          </div>
          <InvoiceTable
