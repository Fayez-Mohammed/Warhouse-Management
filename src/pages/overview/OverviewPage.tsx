...
export function OverviewPage() {
  ...
  const fetchStats = useCallback(
    async (p: DashboardPeriod, from?: string, to?: string) => {
      setLoading(true);
      try {
        ...
        <div className="px-8 py-4 border-b border-border flex items-end gap-3 flex-wrap shrink-0">
          <div className="space-y-1">
            ...
            <input
              ...
              className="bg-input-background border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all"
            />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
          ...
          <button
            ...
            className="bg-primary hover:bg-primary/90 disabled:opacity-40 text-primary-foreground text-sm font-medium px-4 py-2 rounded-lg transition-all"
          >
            {t("btn_apply")}
          </button>
        </div>
