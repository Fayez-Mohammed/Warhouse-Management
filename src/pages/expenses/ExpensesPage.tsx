...
function ExpenseModal({ expense, onClose, onDone }: {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setLoading(true);
        try {
            const body: Record<string, unknown> = { amount: parsed, description };
            ...
            if (data.statusCode === 200) {
                toast.success(isEdit ? "Expense updated." : `Expense #${data.expense?.code} added.`);
                onDone();
            ...
            <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-2xl p-6 space-y-5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-semibold text-foreground">{isEdit ? t("expense_editTitle") : t("expense_newTitle")}</h2>
                            {isEdit && <p className="text-xs text-muted-foreground font-mono">#{expense!.code}</p>}
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-sm font-medium text-foreground/80">{t("lbl_expenseDesc")}</label>
                        <textarea
                            value={description} onChange={e => setDescription(e.target.value)}
                            required rows={3} placeholder={t("placeholder_expenseDesc")}
                    ...
                    <div className="flex gap-3 pt-1">
                        ...
                        >
                            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" />{isEdit ? t("btn_save") : t("btn_addExpense")}</>}
                        </button>
                    </div>
                </form>
            </div>
...
function DeleteModal({ expense, onClose, onDone }: {
    expense: Expense; onClose: () => void; onDone: () => void;
}) {
    const { t } = useLang();
    const [loading, setLoading] = useState(false);
    ...
    async function handleDelete() {
        setLoading(true);
        try {
            const res = await apiFetch(`${API}/api/Expenses?id=${expense.id}`, { method: "DELETE", headers: authHeaders() });
            const data = await res.json();
            if (data.statusCode === 200) { toast.success("Expense deleted."); onDone(); }
    ...
    return (
        <Overlay onClose={onClose}>
            <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-2xl p-6 space-y-5">
                ...
                <div>
                    <h2 className="text-base font-semibold text-foreground">{t("expense_deleteTitle")}</h2>
...
type ModalState = { type: "create" } | { type: "edit"; expense: Expense } | { type: "delete"; expense: Expense };
...
export function ExpensesPage() {
    ...
    const fetchExpenses = useCallback(async (s: number) => {
        setLoading(true);
        try {
            const res = await apiFetch(`${API}/api/Expenses/GetAll?skip=${s}&take=${PAGE_SIZE}`, { headers: authHeaders() });
            const data = await res.json();
    ...
    const runningTotal = expenses.reduce((s, e) => s + e.amount, 0);
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden bg-background/50">
            ...
            <div className="border-b border-border px-8 py-5 flex items-center justify-between shrink-0 bg-background">
                <div>
                    <h1 className="text-lg font-semibold text-foreground">{t("expenses_title")}</h1>
                    <p className="text-xs text-muted-foreground mt-0.5">{t("expenses_subtitle")}</p>
                </div>
                <div className="flex items-center gap-3">
                    {expenses.length > 0 && (
                        <div className="text-end">
                            <p className="text-xs text-muted-foreground">{t("lbl_pageTotal")}</p>
                            <p className="font-mono text-sm font-semibold text-rose-600 dark:text-rose-400"><span dir="ltr" className="inline-block">${runningTotal.toLocaleString()}</span></p>
                        </div>
                    )}
                    <button
                    ...
                    >
                        <Plus className="w-4 h-4" />{t("btn_newExpense")}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto px-8 py-6">
                {loading ? (
                    ...
                    <div className="bg-card border border-border rounded-2xl p-24 flex flex-col items-center justify-center gap-3 shadow-sm">
                        ...
                        <p className="text-muted-foreground text-sm">{t("empty_expenses")}</p>
                        <button onClick={() => setModal({ type: "create" })} className="text-sm text-primary hover:underline">{t("btn_addExpense")}</button>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-border bg-muted/10">
                                    {[
                                        { key: "#", label: "#" },
                                        { key: "desc", label: t("col_description") },
                            ...
                            <tbody>
                                {expenses.map((e, i) => (
                                    <tr key={e.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i === expenses.length - 1 ? "border-b-0" : ""}`}>
                                        <td className="text-start py-3.5 px-5">
                                            <span className="font-mono text-xs bg-rose-500/10 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded" dir="ltr">#{e.code}</span>
                                        </td>
                                        <td className="text-start py-3.5 px-5 text-foreground max-w-xs">{e.description}</td>
                                        <td className="text-start py-3.5 px-5 font-mono text-sm font-semibold text-rose-600 dark:text-rose-400"><span dir="ltr" className="inline-block">${e.amount.toLocaleString()}</span></td>
                                        <td className="text-start py-3.5 px-5 text-xs text-muted-foreground">{e.accountantname}</td>
                                        <td className="text-start py-3.5 px-5 font-mono text-xs text-muted-foreground whitespace-nowrap"><span dir="ltr" className="inline-block">{formatDate(e.createdat)}</span></td>
            ...
            {!loading && expenses.length > 0 && (
                <div className="border-t border-border px-8 py-3.5 flex items-center justify-between shrink-0 bg-background">
                    <p className="text-xs text-muted-foreground font-mono"><span dir="ltr" className="inline-block">{t("pagination_showing")} {skip + 1}–{skip + expenses.length}</span></p>
            ...
            {modal?.type === "create" && <ExpenseModal onClose={() => setModal(null)} onDone={afterMutation} />}
            {modal?.type === "edit" && <ExpenseModal expense={modal.expense} onClose={() => setModal(null)} onDone={afterMutation} />}
            {modal?.type === "delete" && <DeleteModal expense={modal.expense} onClose={() => setModal(null)} onDone={afterMutation} />}
        </div>
    );
}