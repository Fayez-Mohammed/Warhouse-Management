...
export function UserModal({ user, defaultType, onClose, onDone }: {
  user?: AppUser;
  ...
  const [usertype, setUsertype] = useState<AllowedUserType>(
    (ALLOWED_USER_TYPES as readonly string[]).includes(user?.usertype ?? "")
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        const res = await apiFetch(`${API}/api/users/update?id=${user!.id}`, {
          method: "PUT",
          headers: authHeaders(),
          body: JSON.stringify({ fullname, phonenumber, usertype, isactive }),
        });
        const data = await res.json();
        if (data.statusCode === 200) { toast.success("User updated."); onDone(); }
        else toast.error(data.message || "Update failed.");
      } else {
        const res = await apiFetch(`${API}/api/users/create?userType=${usertype}`, {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({ fullname, phonenumber }),
        });
        const data = await res.json();
        if (data.statusCode === 200) { toast.success(`${TYPE_LABELS[usertype]} created.`); onDone(); }
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-foreground">
              {isEdit ? t("modal_editUser") : `${t("btn_new")} ${TYPE_LABELS[defaultType]}`}
            </h2>
            {isEdit && <p className="text-xs text-muted-foreground font-mono mt-0.5">#{user!.usernumber}</p>}
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {isEdit && (
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground/80">{t("lbl_userType")}</label>
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-foreground/80">{t("col_name")}</label>
            <input value={fullname} onChange={e => setFullname(e.target.value)} required placeholder="e.g. Ahmed Mohamed" className={inputCls} />
          </div>

          <div className="space-y-1.5">
            <label className="text-sm font-medium text-foreground/80">{t("lbl_phone")}</label>
          ...
          {isEdit && (
            <label className="flex items-center gap-3 cursor-pointer select-none">
              ...
              <span className="text-sm text-foreground/80">{isactive ? t("status_active") : t("status_inactive")}</span>
            </label>
          )}
