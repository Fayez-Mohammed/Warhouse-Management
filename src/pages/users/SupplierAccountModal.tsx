...
import { X, Loader2, CreditCard, Printer, Eye } from "lucide-react";
...
export function SupplierAccountModal({
  ...
  const [summary, setSummary] =
    useState<SupplierAccountSummary | null>(null);
  ...
  async function load() {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/Invoices/supplier/summary?supplierId=${user.id}`,
        { headers: authHeaders() },
      );
          ...
          <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
            <div>
              ...
              <p className="text-xs text-muted-foreground font-mono mt-0.5">
                Supplier #{user.usernumber} · {user.phonenumber}
              </p>
            </div>
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <SummaryBlock
                    label={t("lbl_supplyCount")}
                    count={summary.supplycount}
                    total={summary.supplytotalamount}
                    paid={summary.supplypaidamount}
                    remaining={summary.supplyremainingamount}
                    color="bg-amber-500/5"
                  />
                  <SummaryBlock
                    label={t("lbl_returnsCount")}
                        ...
                        <thead>
                          <tr className="border-b border-border bg-muted/20">
                            {[
                              ...
                              <th
                                key={h}
                                className={`text-left py-2.5 px-2.5 text-xs font-medium text-muted-foreground font-mono tracking-wider ${!h ? "print-hidden" : ""}`}
                              >
                                {h}
                              </th>
                            ))}
