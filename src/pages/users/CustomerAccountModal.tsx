...
import { X, Loader2, TrendingDown, TrendingUp, RotateCcw } from "lucide-react";
...
function InvoiceRow({ inv }: { inv: CustomerSummaryInvoice }) {
  ...
  return (
    <tr className="border-b border-border/40 hover:bg-muted/20 transition-colors">
      ...
      {inv.ordercode && <td className="py-2.5 px-2.5"><span className="font-mono text-xs bg-muted px-1.5 py-0.5 rounded text-muted-foreground">ord #{inv.ordercode}</span></td>}
    </tr>
  );
}
      ...
      <div className="w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground font-mono mt-0.5">Customer #{user.usernumber} · {user.phonenumber}</p>
          </div>
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <SummaryRow
                  label={t("lbl_salesCount")} count={summary.salescount}
              ...
              <div className="bg-muted/30 rounded-xl p-4 border border-border flex items-center justify-between">
                ...
                <span className={`font-mono text-lg font-semibold ${
                  (summary.salesremainingamount - summary.returnsremainingamount) > 0
