...
import type { AppUser, CustomerAccountSummary, CustomerSummaryInvoice } from "../../types";

function SummaryRow({ label, count, total, paid, remaining, color }: {
    ...
    return (
        <div className={`rounded-xl border border-border p-4 space-y-2 ${color}`}>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground/75">{t("col_amount")}</p>
                    <p className="font-mono text-sm font-semibold text-foreground">${total.toLocaleString()}</p>
                </div>
                <div>
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground/75">{t("col_paid")}</p>
                    <p className="font-mono text-sm font-semibold text-emerald-600 dark:text-emerald-400">${paid.toLocaleString()}</p>
                </div>
                <div>
                    <p className="text-[10px] uppercase font-semibold text-muted-foreground/75">{t("col_remaining")}</p>
...
function InvoiceRow({ inv }: { inv: CustomerSummaryInvoice }) {
    const { t } = useLang();
    ...
    return (
        <tr className="border-b border-border/40 hover:bg-muted/20 transition-colors">
            ...
            <td className="py-2.5 px-2.5">
                ...
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border whitespace-nowrap ${tc.bg} ${tc.color} ${tc.border}`}>
                    {t(tc.labelKey)}
                </span>
            </td>
            <td className="py-2.5 px-2.5 font-mono text-xs text-foreground">${inv.amount.toLocaleString()}</td>
...
export function CustomerAccountModal({ user, onClose }: { user: AppUser; onClose: () => void }) {
    const { t } = useLang();
    ...
    useEffect(() => {
        async function load() {
            try {
                const res = await apiFetch(`${API}/api/Invoices/customer/summary?customerId=${user.id}`, { headers: authHeaders() });
                ...
                <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground font-mono mt-0.5">
                            {t("tab_customers")} #{user.usernumber} · {user.phonenumber}
                        </p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <SummaryRow
                                    label={t("tab_salesReps")} count={summary.salescount}
                                    total={summary.salestotalamount} paid={summary.salespaidamount} remaining={summary.salesremainingamount}
                                    color="bg-sky-500/5"
                                />
                                <SummaryRow
                                    label={t("tab_suppliers")} count={summary.returnscount}
                            ...
                            <div className="bg-muted/30 rounded-xl p-4 border border-border flex items-center justify-between">
                                <span className="text-sm font-medium text-foreground">{t("lbl_netReceivable")}</span>
                                <span className={`font-mono text-lg font-semibold ${(summary.salesremainingamount - summary.returnsremainingamount) > 0
                                        ? "text-destructive" : "text-emerald-600 dark:text-emerald-400"
                                    }`}>
                                    ${(summary.salesremainingamount - summary.returnsremainingamount).toLocaleString()}
                                </span>
                            </div>
                                            ...
                                            <thead>
                                                <tr className="border-b border-border bg-muted/20">
                                                    {["#", t("col_type"), t("col_amount"), t("col_paid"), t("col_remaining"), t("col_date")].map(h => (
                                                        <th key={h} className="text-left py-2.5 px-2.5 text-xs font-semibold text-muted-foreground font-mono tracking-wider">{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {summary.invoices.map(inv => <InvoiceRow key={inv.id} inv={inv} />)}
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
