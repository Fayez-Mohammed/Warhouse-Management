...
export function InvoiceProductsModal({
  invoice,
  onClose,
}: {
    ...
    async function load() {
      try {
        const res = await apiFetch(
          `${API}/api/Invoices/supplier/products?invoiceId=${invoice.id}`,
          { headers: authHeaders() },
        );
        const data = await res.json();
        setProducts(data.value ?? []);
      } catch {
        toast.error("Failed to load invoice products.");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="print-document w-full max-w-lg bg-card border border-border rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        <div className="px-6 py-4 border-b border-border flex items-start justify-between shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Package className="w-4 h-4 text-muted-foreground" />
              <h2 className="text-base font-semibold text-foreground">
                {t("modal_invoiceProducts")} #{invoice.code}
              </h2>
            </div>
          ...
          <div className="print-hidden flex items-center gap-2 shrink-0 ml-4">
            <button
            ...
            <table className="w-full min-w-[420px] text-sm">
              <thead>
                <tr className="border-b border-border">
                  {[
                    ...
                    <th
                      key={h}
                      className="text-left py-2.5 px-2 text-xs font-medium text-muted-foreground font-mono tracking-wider"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tfoot>
                <tr className="border-t-2 border-border">
                  <td
                    colSpan={4}
                    className="py-3 px-2 text-xs font-medium text-muted-foreground text-right"
                  >
                    {t("col_total")}
                  </td>
                  <td className="py-3 px-2 font-mono font-semibold text-foreground">
