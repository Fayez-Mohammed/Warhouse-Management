  ...
  async function handlePay() {
    ...
    try {
      ...
      if (data.statusCode === 200) {
        toast.success(
          `Paid $${parsed.toLocaleString()} to ${data.supplier ?? invoice.recipientname}. Remaining: $${data.remainingamount?.toLocaleString()}`,
        );
        onDone();
      } else {
      ...
      <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-2xl p-6 space-y-5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center ${typeConfig.bg} ${typeConfig.color}`}
            >
