...
export function InventoryCheckPage() {
  ...
  const [preview, setPreview] = useState<AdjustResult | null>(
    null,
  );
  ...
  async function handleSimulate() {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/InventoryCheck/Adjust?UpdateStock=false`,
        {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({
  ...
  async function handleApply() {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/InventoryCheck/Adjust?UpdateStock=true`,
        {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({
  ...
  const diff = preview
    ? Number(actualQty) - preview.systemquantity
    : 0;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={
                !productName.trim() || !actualQty || loading
              }
        ...
        <div
          className={`bg-card border rounded-xl p-6 space-y-5 transition-all ${preview ? "border-border opacity-100" : "border-dashed border-border opacity-60"}`}
        >
          <h2 className="text-sm font-semibold text-foreground">
            {t("preview_title")}
          </h2>
          ...
          {preview && (
            <>
              <div className="grid grid-cols-3 gap-3">
                ...
                <div
                  className={`rounded-lg p-3 text-center ${isSurplus ? "bg-emerald-500/10" : isDeficit ? "bg-destructive/10" : "bg-primary/10"}`}
                >
                  <p className="text-xs text-muted-foreground mb-1">
                    {t("lbl_difference")}
                  </p>
                  <p
                    className={`text-xl font-bold font-mono ${isSurplus ? "text-emerald-500" : isDeficit ? "text-destructive" : "text-primary"}`}
                  >
                    <span dir="ltr" className="inline-block">
                      {isSurplus ? `+${diff}` : diff}
                    </span>
              ...
              >
                ...
                <div>
                  <p
                    className={`text-sm font-semibold ${isSurplus ? "text-emerald-500" : isDeficit ? "text-destructive" : "text-primary"}`}
                  >
                    {isSurplus
                      ? t("status_surplus")
                      : isDeficit
              ...
              <div className="flex items-center justify-between bg-muted/40 rounded-lg px-4 py-3">
                <span className="text-xs text-muted-foreground uppercase tracking-wide">
                  {t("lbl_financialImpact")}
                </span>
                <span
                  className={`text-sm font-semibold font-mono ${preview.valuedifference >= 0 ? "text-emerald-500" : "text-destructive"}`}
                >
                  <span dir="ltr" className="inline-block">
                    {preview.valuedifference >= 0 ? "+" : ""}
                    {preview.valuedifference.toLocaleString()}{" "}
                    EGP
                  </span>
                </span>
              </div>
              ...
              {(applied || isMatch) && (
                <button
                  onClick={handleReset}
                  className="w-full px-4 py-2.5 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all"
                >
                  {t("btn_newCheck")}
                </button>
              )}
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          <p>
            <span className="font-medium text-foreground">
              {t("invCheck_previewMode")}
            </span>{" "}
            {t("invCheck_previewModeDesc")}
          </p>
          <p>
            <span className="font-medium text-foreground">
              {t("invCheck_applyLabel")}
            </span>{" "}
            {t("invCheck_applyLabelDesc")}
          </p>
        </div>
      </div>
    </div>
