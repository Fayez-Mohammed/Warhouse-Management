                ...
                {loading ? (
                    ...
                    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {orders.map((o, i) => (
                                    <tr key={o.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i === orders.length - 1 ? "border-b-0" : ""}`}>
                                        <td className="text-start py-3.5 px-5"><span className="font-mono text-xs bg-primary/10 text-primary px-2 py-0.5 rounded" dir="ltr">#{o.code}</span></td>
                                        <td className="text-start py-3.5 px-5 font-medium text-foreground">{o.customername}</td>
                                        <td className="text-start py-3.5 px-5 text-muted-foreground text-xs">{o.salesrepname ?? <span className="italic opacity-50">—</span>}</td>
                                        <td className="text-start py-3.5 px-5 font-mono text-sm font-semibold text-emerald-600 dark:text-emerald-400"><span dir="ltr" className="inline-block">${o.totalamount.toLocaleString()}</span></td>
                                        <td className="text-start py-3.5 px-5 font-mono text-xs text-muted-foreground"><span dir="ltr" className="inline-block">{o.commissionamount > 0 ? `$${o.commissionamount.toFixed(2)}` : "—"}</span></td>
                                        ...
                                        <td className="text-start py-3.5 px-5 text-muted-foreground font-mono text-xs"><span dir="ltr" className="inline-block">{formatDate(o.dateofcreation)}</span></td>
