...
export function OrdersPage() {
  ...
  const [returnOrder, setReturnOrder] = useState<Order | null>(
    null,
  );
  ...
  const fetchOrders = useCallback(async (s = 0) => {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/Orders/approved?skip=${s}&take=${PAGE_SIZE}`,
        { headers: authHeaders() },
      );
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-background/50">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/10">
                  {[
                    ...
                    {
                      key: "col_salesRep",
                      label: t("col_salesRep"),
                    },
                    { key: "col_total", label: t("col_total") },
                    {
                      key: "col_commission",
                      label: t("col_commission"),
                    },
                    {
                      key: "col_status",
                      label: t("col_status"),
              ...
              <tbody>
                {orders.map((o, i) => (
                  <tr
                    key={o.id}
                    className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i === orders.length - 1 ? "border-b-0" : ""}`}
                  >
                    ...
                    <td className="text-start py-3.5 px-5 font-mono text-sm font-semibold text-emerald-600 dark:text-emerald-400">
      ...
      {payOrder && (
        <CollectPaymentModal
          ...
          onDone={() => setPayOrder(null)}
        />
      )}
      {showCreate && (
        <CreateDirectOrderModal
