...
export function CollectPaymentModal({
  ...
  const { t } = useLang();
  const [tab, setTab] = useState<"Customer" | "SalesRep">(
    "Customer",
  );
  const [customerInfo, setCustomerInfo] =
    useState<PaymentInfo | null>(null);
  const [salesRepInfo, setSalesRepInfo] =
    useState<PaymentInfo | null>(null);
    ...
    async function load() {
      ...
      try {
        const [cRes, sRes] = await Promise.all([
          apiFetch(
            `${API}/api/Invoices/customer/remaining-amount?orderId=${order.id}`,
            { headers: authHeaders() },
          ),
          apiFetch(
            `${API}/api/Invoices/salesrep/remaining-amount?orderId=${order.id}`,
            { headers: authHeaders() },
          ),
        ]);
        ...
        if (cData.statusCode === 200) {
          setCustomerInfo({
            ...
            remainingamount: cData.remainingamount,
            name: cData.customername ?? order.customername,
          });
        }
        if (sData.statusCode === 200) {
          setSalesRepInfo({
            total: sData.total,
            paidamount: sData.paidamount,
            remainingamount: sData.remainingamount,
            name:
              sData.salesrepname ??
              order.salesrepname ??
              "Sales Rep",
  ...
  const activeInfo =
    tab === "Customer" ? customerInfo : salesRepInfo;
      ...
      if (data.statusCode === 200) {
        const name =
          tab === "Customer"
            ...
            : (data.salesrepname ?? order.salesrepname);
        toast.success(
          `$${parsed.toLocaleString()} collected from ${name}. Remaining: $${data.remainingamount?.toLocaleString()}`,
        );
        onDone();
      } else {
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  ...
                  <p className="font-mono text-sm font-medium text-foreground">
                    ${activeInfo.total.toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">
                  ...
                  <p className="font-mono text-sm font-medium text-emerald-600 dark:text-emerald-400">
                    ${activeInfo.paidamount.toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">
                  ...
                  <p
                    className={`font-mono text-sm font-medium ${activeInfo.remainingamount > 0 ? "text-destructive" : "text-emerald-600 dark:text-emerald-400"}`}
                  >
                    $
              ...
              {activeInfo.total > 0 && (
                <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min((activeInfo.paidamount / activeInfo.total) * 100, 100)}%`,
                    }}
                  />
                </div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                {amount && !valid && (
                  <p className="text-xs text-destructive">
                    Enter a valid amount between $0.01 and $
                    {max.toLocaleString()}
                  </p>
                )}
              </div>
