  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiFetch(
        `${API}/api/Inventory/Listproducts/stock/in?supplierName=${encodeURIComponent(supplierName)}`,
        {
          method: "POST",
          headers: authHeaders(),
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Add stock to existing products — updates quantity
              & buy price
            </p>
          </div>
          <button
        ...
        >
          ...
          <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
            {rows.map((row, i) => (
              <div
                key={i}
                className="grid grid-cols-4 gap-2 items-end p-3 bg-muted/30 rounded-xl border border-border/50"
              >
