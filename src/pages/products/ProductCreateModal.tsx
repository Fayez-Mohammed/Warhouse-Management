  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiFetch(
        `${API}/api/Inventory/ListOfProductsWithCategoryName?supplierName=${encodeURIComponent(supplierName)}`,
        {
          method: "POST",
          headers: authHeaders(),
      ...
      if (data.statusCode === 200) {
        ...
        toast.error(
          data.message || "Failed to create products.",
        );
    } catch {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Bulk insert — auto-creates stock movement &
              supplier invoice
            </p>
          </div>
          <button
        ...
        >
          ...
          <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
            {rows.map((row, i) => (
              <div
                key={i}
                className="grid grid-cols-7 gap-2 items-end p-3 bg-muted/30 rounded-xl border border-border/50"
              >
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    value={row.saleprice}
                    onChange={(e) =>
                      updateRow(i, "saleprice", e.target.value)
                    }
                    required
                    placeholder="0"
                    className={inputCls}
                ...
                <div className="flex items-end gap-2">
                  ...
                  {rows.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeRow(i)}
                      className="w-8 h-9 shrink-0 flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all"
                    >
