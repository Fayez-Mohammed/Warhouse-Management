...
import { useLang } from "../../lib/i18n"; // <--- استيراد الـ hook الخاص بالترجمة
...
export function ReturnToSupplierModal({
  ...
  const { t } = useLang(); // <--- تفعيل دالة الترجمة t
  ...
  const inputCls =
    "w-full bg-input-background border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiFetch(
        `${API}/api/Returns/returntosupplier`,
        {
          method: "POST",
          headers: authHeaders(),
      ...
      if (data.statusCode === 200) {
        ...
        toast.success(
          refund != null
            ? `${t("lbl_stockUpdated")}. ${t("lbl_amount")}: ${refund.toLocaleString()} EGP — ${t("col_invNo")} #${inv}`
            : t("lbl_stockUpdated"),
        );
        onDone();
      } else {
        toast.error(data.message || t("lbl_noData"));
      }
    } catch {
      toast.error(t("lbl_noData"));
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            <h2 className="text-base font-semibold text-foreground flex items-center gap-2">
              ...
              {t("modal_returnToSupplier")}
            </h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("modal_returnToSupplierSub")}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        >
          ...
          <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
                ...
                <div className="flex items-end justify-end">
                  {rows.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeRow(i)}
                      className="w-8 h-9 flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all"
                    >
            ...
            <button
              type="button"
              onClick={addRow}
              className="w-full border border-dashed border-border rounded-xl py-2.5 text-sm text-muted-foreground hover:text-foreground hover:border-amber-500/40 transition-all flex items-center justify-center gap-2"
            >
              <Plus className="w-4 h-4" />
              {t("btn_addAnotherItem")}
            </button>
          </div>
