          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-start py-3 px-4 text-xs font-medium text-muted-foreground font-mono tracking-wider">{t("col_code")}</th>
                <th className="text-start py-3 px-4 text-xs font-medium text-muted-foreground font-mono tracking-wider">{t("col_name")}</th>
                <th className="text-start py-3 px-4 text-xs font-medium text-muted-foreground font-mono tracking-wider">{t("col_description")}</th>
                <th className="text-start py-3 px-4 text-xs font-medium text-muted-foreground font-mono tracking-wider">{t("col_created")}</th>
                <th className="py-3 px-4 w-20" />
              </tr>
            </thead>
            <tbody>
              {categories.map((cat, i) => (
                <tr key={cat.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i % 2 === 0 ? "" : "bg-card/30"}`}>
                  <td className="text-start py-3.5 px-4">
                    <span className="font-mono text-xs bg-primary/10 text-primary px-2 py-0.5 rounded" dir="ltr">#{cat.code}</span>
                  </td>
                  <td className="text-start py-3.5 px-4 font-medium text-foreground">{cat.name}</td>
                  <td className="text-start py-3.5 px-4 text-muted-foreground">{cat.description ?? <span className="italic text-muted-foreground/40">—</span>}</td>
                  <td className="text-start py-3.5 px-4 text-muted-foreground font-mono text-xs"><span dir="ltr" className="inline-block">{formatDate(cat.dateofcreation)}</span></td>
