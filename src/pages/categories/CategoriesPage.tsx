...
export function CategoriesPage() {
  ...
  const fetchCategories = useCallback(async (s = 0) => {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/Categories/GetAllCategories?skip=${s}&take=${PAGE_SIZE}`,
        { headers: authHeaders() },
      );
  ...
  useEffect(() => {
    ...
    acTimer.current = setTimeout(async () => {
      try {
        const res = await apiFetch(
          `${API}/api/Categories/autocomplete?term=${encodeURIComponent(search)}`,
          { headers: authHeaders() },
        );
        const data = await res.json();
  ...
  async function handleSearchSelect(name: string) {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/Categories/GetCategoryByName?name=${encodeURIComponent(name)}`,
        { headers: authHeaders() },
      );
      const data = await res.json();
            ...
            <table className="w-full min-w-[720px] text-sm">
              ...
              <tbody>
                {categories.map((cat, i) => (
                  <tr
                    key={cat.id}
                    className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i === categories.length - 1 ? "border-b-0" : ""}`}
                  >
                    ...
                    <td className="text-start py-3.5 px-6 font-medium text-foreground">
                      {cat.name}
                    </td>
                    <td className="text-start py-3.5 px-6 text-muted-foreground">
                      {cat.description ?? (
                    ...
                    <td className="text-start py-3.5 px-6 text-muted-foreground font-mono text-xs">
                      <span dir="ltr" className="inline-block">
                        {formatDate(cat.dateofcreation)}
                      </span>
                    </td>
