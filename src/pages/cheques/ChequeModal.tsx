    ...
    timer.current = setTimeout(async () => {
      try {
        const res = await apiFetch(
          `${API}/api/Invoices/autocomplete?term=${encodeURIComponent(value)}`,
          { headers: authHeaders() },
        );
...
export function ChequeModal({
  ...
  const [amount, setAmount] = useState(
    cheque ? String(cheque.amount) : "",
  );
  const [duedate, setDuedate] = useState(
    cheque ? cheque.duedate.split("T")[0] : "",
  );
  ...
  async function fetchInvoicesForName(
    name: string,
    incoming: boolean,
  ) {
    if (!name.trim()) return;
  ...
  function handlePersonSelect(name: string) {
    fetchInvoicesForName(name, isincoming);
  }
  ...
  function handleDirectionChange(incoming: boolean) {
    ...
    setSelectedInvoiceId("");
    if (relatedname.trim())
      fetchInvoicesForName(relatedname, incoming);
  }
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-base font-semibold text-foreground">
              {isEdit
                ? t("cheques_title") + " — " + t("lbl_editItem")
                : t("btn_newCheque")}
            </h2>
            {isEdit && (
            ...
            <div className="grid grid-cols-2 gap-2">
              {[true, false].map((incoming) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    handleDirectionChange(incoming)
                  }
          ...
          {relatedname.trim() && (
            <div className="space-y-1.5">
              ...
              {selectedInvoice && (
                ...
                >
                  <span>
                    Invoice #{selectedInvoice.code} · Total $
