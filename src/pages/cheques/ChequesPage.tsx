                ...
                <div className="flex items-center gap-1 bg-muted/40 rounded-lg p-1 border border-border">
                    {([["", t("dir_all")], ["Incoming", t("dir_incomingShort")], ["Outgoing", t("dir_outgoingShort")]] as const).map(([v, lbl]) => (
                        <button key={v} onClick={() => setDirFilter(v)}
                            className={`px-3 py-1 rounded-md text-xs font-medium transition-all flex items-center gap-1.5 ${dirFilter === v ? "bg-card text-foreground shadow-sm border border-border" : "text-muted-foreground hover:text-foreground"
                                }`}
                        >
                ...
                <div className="flex items-center gap-1 bg-muted/40 rounded-lg p-1 border border-border">
                    {["", "Pending", "Collected", "Paid", "Rejected", "Cancelled"].map(v => (
                        <button key={v} onClick={() => setStatusFilter(v)}
                            className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${statusFilter === v ? "bg-card text-foreground shadow-sm border border-border" : "text-muted-foreground hover:text-foreground"
                                }`}
                        >
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {cheques.map((c, i) => {
                                    ...
                                    return (
                                        <tr key={c.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i === cheques.length - 1 ? "border-b-0" : ""}`}>
                                            ...
                                            <td className="text-start py-3.5 px-5 font-mono text-xs text-foreground"><span dir="ltr" className="inline-block">{c.checknumber}</span></td>
                                            <td className="text-start py-3.5 px-5 font-medium text-foreground max-w-[140px] truncate">{c.relatedname}</td>
                                            <td className="text-start py-3.5 px-5 text-xs text-muted-foreground">{c.bankname}</td>
                                            <td className="text-start py-3.5 px-5 font-mono text-sm font-medium text-foreground"><span dir="ltr" className="inline-block">${c.amount.toLocaleString()}</span></td>
                                            <td className="text-start py-3.5 px-5 font-mono text-xs text-muted-foreground whitespace-nowrap"><span dir="ltr" className="inline-block">{formatDate(c.duedate)}</span></td>
                                            <td className="text-start py-3.5 px-5">
                                            ...
                                            <td className="text-start py-3.5 px-5 font-mono text-xs text-muted-foreground whitespace-nowrap"><span dir="ltr" className="inline-block">{formatDate(c.issuedate)}</span></td>
