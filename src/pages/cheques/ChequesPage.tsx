...
export function ChequesPage() {
  ...
  function goPage(dir: "prev" | "next") {
    ...
    setSkip(s);
    fetchCheques(s, search, dirFilter, statusFilter);
  }
      ...
      <div className="px-8 py-3.5 border-b border-border shrink-0 flex items-center gap-3 flex-wrap bg-background">
        ...
        <div className="flex items-center gap-1 bg-muted/40 rounded-lg p-1 border border-border">
          {(
            [
              ["", t("dir_all")],
              ["Incoming", t("dir_incomingShort")],
              ["Outgoing", t("dir_outgoingShort")],
            ] as const
          ).map(([v, lbl]) => (
            <button
              key={v}
      ...
      <div className="flex-1 overflow-auto px-8 py-6">
        {loading ? (
          ...
          <div className="bg-card border border-border rounded-2xl p-24 flex flex-col items-center justify-center gap-3 shadow-sm">
            <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center text-muted-foreground text-lg font-bold">
              ✓
            </div>
              ...
              <tbody>
                {cheques.map((c, i) => {
                  ...
                  return (
                    <tr
                      key={c.id}
                      className={`border-b border-border/50 hover:bg-muted/20 transition-colors group ${i === cheques.length - 1 ? "border-b-0" : ""}`}
                    >
                      ...
                      <td className="text-start py-3.5 px-5 font-mono text-xs text-foreground">
                        <span
                          dir="ltr"
                      ...
                      <td className="text-start py-3.5 px-5 font-mono text-sm font-medium text-foreground">
                        <span
                          dir="ltr"
                          className="inline-block"
                        >
                      ...
                      <td className="text-start py-3.5 px-5">
                        <span
                          className={`flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border w-fit ${sc.bg} ${sc.color} ${sc.border}`}
                        >
                          {sc.icon}
                          {t(
                            `status_${c.status.toLowerCase()}`,
                          )}
                        </span>
                      </td>
                      <td className="text-start py-3.5 px-5 font-mono text-xs text-muted-foreground whitespace-nowrap">
